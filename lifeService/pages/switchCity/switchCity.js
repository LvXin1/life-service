...
Page({
  ...
  clickLetter(e){
    ...
    this.data.searchLetter.forEach((row,index) => {
      //console.log(row)
      if (showLetter == row.name && index != 0){count = index;}
    })
    var scrollTop = 50 * count;
    var index = 0
    while (index < count){
      console.log(this.data.cityList[index].cityInfo.length)
      allCount += this.data.cityList[index].cityInfo.length;
      index ++;
    }
    console.log(count)
    this.setData({
      scrollTop: (scrollTop + 100 * allCount) / 2 + 240
    })
  },
  /**
   * 用户点击右上角分享
